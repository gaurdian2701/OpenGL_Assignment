...
{
public:
	LoggedMesh(VertexData vertexData, std::vector<unsigned int> indices, std::vector<Texture> textures);
	...
	void SetupMesh();
	std::vector<std::pair<std::string, float>>& GetLogData();
...
private:
	...
	std::vector<std::pair<std::string, float>> m_loggedData;
};
